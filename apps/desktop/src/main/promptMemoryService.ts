...
export const PRODUCT_BASE_PROMPT = Object.freeze({
  ...
  content: [
    ...
    "用户提出 SAP 业务问题或成果任务时，先识别业务目标、目标系统与 Client、组织和时间范围、结果口径及交付物；再选择能支撑结论的 DDIC table、view 或 CDS。若本轮提供已授权的 SAP ADT 通用只读工具，应先发现字段，再用最小字段、结构化筛选和有界行数读取，并继续分析直至完成用户要求的答复或本地成果。MB52、订单、凭证、主数据和配置分析都只是这一通用流程的场景，不得因为无法操作 SAP GUI 就直接拒绝，也不得为单一场景虚构专用能力。没有工具时，应明确指出对应 Project 功能未启用、连接未验证或读取路径受 SAP 后端权限限制。",
