...
| 词汇 | 含义 |
|---|---|
| Project | 项目边界，用来隔离客户、SAP 版本、连接配置、项目规范、案件和知识。 |
...
| Work | 需要沉淀成果的正式工作入口，必须绑定一个工作文件夹。当前实现里 Work 使用 Project + Case 保存。 |
| Chat | 纯日常对话入口，不出现项目选择、文件夹选择和案件文件面板，也不读取 SAP。 |
| SAP 项目 | 已配置或准备配置 SAP 连接、规范、知识和模型能力的 Project，例如 `SAP 演示 S4HANA`、`SAP 演示 ECC`。 |
| 工作文件夹 | 用户在 Work 下看到的文件夹概念，当前实现对应底层 Case。它必须归属到某个 SAP 项目或“其他工作”项目。 |
...
- 主界面必须区分 `Work` 和 `Chat`：`Chat` 不显示文件夹选择；`Work` 下新建内容必须选择或创建工作文件夹。
