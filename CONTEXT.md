...
| 对话回退 / Conversation Rewind | 编辑历史用户消息时，把当时的完整对话保存为可恢复版本，再从目标消息处截断当前对话并重发。它只改变当前线程的聊天上下文，不删除或回滚运维项目共享文件。 |
...
- 编辑历史消息必须先保留可恢复的对话版本，再回退当前线程；回退后要撤销旧 checkpoint，防止被截断内容重新进入模型上下文。运维项目共享文件属于独立成果，不得随聊天回退自动删除或覆盖。
