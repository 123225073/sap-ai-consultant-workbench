...
> 实现状态说明：本文同时保留产品目标与当前基线。没有标注“当前已实现”的扩展能力均按第二阶段目标理解，不能用于宣称 0.1.0 已经可用。0.1.0 的真实边界以 `README.md`、`TECHNICAL_IMPLEMENTATION.md` 开头的状态说明和 `docs/release/KNOWN_LIMITATIONS.md` 为准。
...
| 场景 | 用户目标 | MVP 必须支持 |
|---|---|---|
| SAP 问题分析 | 输入自然语言问题，结合用户明确触发的 SAP 只读取证输出结论；自然语言自动选择连接和取数后置 | 当前支持人工触发只读取证 |
| ABAP 开发 | 按项目规范生成或修改 ABAP 代码、说明和请求描述 | 是 |
| 文档生成 | 基于案件上下文生成开发说明书、技术说明和本地飞书交接草稿 | 是；云端发布后置 |
| 画流程图 | 基于业务逻辑生成 Mermaid 流程图和本地交付文件 | 是；飞书白板发布后置 |
...
4. 任务与工作文件夹管理：一个正式问题、需求或交付事项对应一个独立任务会话；任务可绑定新建或已有工作文件夹，工作文件夹对应底层 Case。
...
9. 配置中心：ADT、飞书 CLI、API 渠道和模型、可选本机增强、本地存储、动作与 Skill 管理的配置、测试、验证和状态显示。
10. 规范中心：项目级规范副本、模板迁移、差异查看、版本保存。
11. 知识库：当前支持 Markdown/TXT 文档导入、候选知识、人工确认、冲突检测和时间线元数据；结构化 QA 批量导入后置。
12. 模型选择器：按 API 渠道分组选择模型，如 Fengsha API 中转、DeepSeek、智谱开放平台。
13. 本地文件沉淀：所有中间产物和最终产物都保存到案件目录。
14. 全局搜索：检索案件、SAP 对象、文档、逻辑图、知识卡片和 QA。
15. 独立 Agent Runtime：核心对话和案件动作由本产品自己的 Thread/Turn/Item、工具、上下文和权限策略运行时承载，不要求安装 Codex；交互式审批流程属于后续能力。
...
| SAP 项目 | 一个客户或业务项目的 SAP landscape、规范、知识和模型配置父级；可包含多个 SID 与 Client | 不直接绑定文件夹 | 可配置只读读取 | 点击配置时显示项目配置 |
| 任务 | Work 下的独立会话，有稳定会话 ID，可归档、移除和恢复 | 是 | 继承所绑定文件夹的 Project | 当前任务对话与绑定文件夹 |
| 工作文件夹 | Work 任务绑定的本地成果容器；可由工作台新建，或通过系统目录选择器关联电脑已有文件夹 | 是 | 继承所属 SAP 项目 | 文件、输出、候选知识、技术材料 |
...
- Work / Chat 分段切换。
- Work 下显示：新建任务、搜索、配置中心、规范中心、知识库。
...
- 每个可见项目允许：
  - 切换项目。
  - 打开项目配置。
  - 点击“新建任务”，并选择新建工作文件夹或通过 Windows 原生目录选择器绑定电脑已有文件夹。
  - 从当前可见列表移除。
...
- 每个 SAP 项目下显示该项目最近任务，任务副标题显示绑定的工作文件夹。
- 任务过多时侧边栏内部滚动。
- 每个任务有稳定会话 ID，并支持归档、移除、恢复和搜索。
- 同一工作文件夹可以绑定多个任务，但任务消息和文件级会话快照必须隔离。
- “已有文件夹”不列出页面内部 Case；用户必须从电脑选择目录。相同 Project 下重复选择同一真实路径时复用同一 Case。
- 文件夹绝对路径属于机器本地敏感元数据，只允许 main process 保存；renderer、模型上下文、日志和工作区迁移备份不得包含它。
- 绑定动作只确认目录身份，不等于批量读取或改写原目录；后续读取单个文件仍需走受控文件能力。
- 不在任务列表中展示技术对象清单。
...
- SSL 设置。
- 只读模式显示。
- 同一 Project 可纵向保存多套 SAP 登录连接，并按 SID 分组展示；不设置面向用户的固定数量上限。
- 可从本机 `SAPUILandscape.xml`、`SAPUILandscapeGlobal.xml` 或 `saplogon.ini` 只读识别主机、SID 和实例号；不得读取或导入 SAP 密码。
- 新建连接默认使用“跳过证书校验”，用户可以主动改成严格校验。
- Work 根据当前问题中的 SID、Client、环境、用途和关键词建议连接；无法唯一判断或需要跨系统读取时必须先确认。
- 保存配置。
- 测试连接。
- 读取 T000 验证。
...
每套 SAP 连接的配置、验证状态和密码引用必须独立；自动识别 SAP GUI 主机时只生成 HTTPS ADT 候选，不得携带密码自动回退到 HTTP。主机没有匹配本机 SAP Logon 且用户没有填写实例号时，不得静默假设实例 `00`。
...
- 显示当前 Profile。
- 验证登录状态。
- 验证当前 Profile 的基础 scope；当前版本不创建或更新云端文档。
- 登录失败时提示用户重新授权。
...
- Work、Chat、案件动作、Skill、MCP 和后续子 Agent 由本产品自己的 Agent Runtime 运行，不通过 Codex CLI、Codex SDK 或 Codex app-server 中转。
- 每个任务/对话有稳定 Thread ID；每次发送形成持久化 Turn，并保存用户消息、模型输出、工具调用、结果和失败状态。应用异常关闭时，未完成 Turn 标记为中断并在原会话显示提示，不自动重放原请求或工具。
- 对话达到上下文阈值时可自动压缩，但完整本地历史不得被压缩摘要覆盖或删除。
- 记忆只用于恢复工作和复用用户确认的约定；正式知识仍走候选知识与人工审核。
- Skill 采用渐进式加载；MCP 当前按 Server 和 Project 配置、测试和发现，外部工具不进入执行链，二者都不能绕过 main process 权限。
- 子 Agent 属于后续能力；开放前必须满足任务可独立、可验证、默认只读、有限并发和最大深度 1。

Codex CLI 可暂时保留为兼容期本机工程辅助：

- 自动扫描、版本显示、登录状态和只读诊断可以继续存在，但默认停用。
- 未安装时不得主动要求用户安装，也不得影响核心功能。
- 只有用户明确选择启用兼容辅助时，才允许经过原生确认执行固定官方安装命令。
- 本工具保存自己的案件和对话，不读取 Codex App 内部数据库。
...
当前 0.1.0 已实现 Skill 包校验、导入、确认、启停和按范围注入上下文；案件动作仍是产品内置固定动作。下面的“动作绑定与自定义”是第二阶段目标，不是当前能力。

第二阶段的动作与 Skill 管理用于统一管理主工作台里的案件动作按钮，以及这些按钮背后绑定的内置或导入 Skill。

必须支持两类 Skill：

| 类型 | 说明 | MVP 操作 |
|---|---|---|
| 内置 Skill | 随系统提供的稳定能力说明；当前案件动作仍由产品代码固定实现。 | 启用、停用、查看、按范围注入上下文。 |
| 导入 Skill | 用户从本地文件夹导入的 Skill。用户可以用外部工具创建或复制 Skill，本工具不负责创建器。 | 导入、校验、确认、启停、查看、移除。 |

导入 Skill 的流程：

```text
点击「导入 Skill」
  -> 选择本地 Skill 文件夹（受控 zip 解压后置）
  -> 检查是否包含 SKILL.md
...
当前 0.1.0 只支持受控导入 UTF-8 Markdown 和 TXT；导入内容先进入候选知识，必须人工审核后发布。
...
每个案件都有一个由工作台维护的真实本地成果目录。用户选择电脑已有文件夹时，系统额外保存一条机器本地绑定关系；原目录不会因绑定被自动遍历、复制或改写。
...
7. 能把生成的 Markdown 开发说明书、Mermaid 流程图、CSV 摘要等当前支持成果保存到案件文件夹。
8. 能在右侧文件面板看到当前案件文件。
9. 能用全局搜索找回历史案件和交付物。
10. 能编辑项目规范，并被案件动作读取使用。
11. 能导入 Skill、校验 `SKILL.md`、确认后启停并按范围进入新回合；案件动作绑定后置。
...
19. 每个模型回合都写入本地 Thread/Turn/Item；应用重启后未完成回合显示中断提示，工具调用和失败不能只存在于界面临时状态。
20. 长会话可以用本地确定性 checkpoint 继续，完整原始历史不删除；checkpoint 查看、重建和手动生成界面属于后续增强。
21. Skill、MCP 以及后续子 Agent 不得绕过 SAP 只读、Project/Case 隔离、密钥保护和外部发布确认。
...
| 导入 Skill 执行风险 | 当前只加载声明文本和资源，导入包脚本不执行；后续开放执行前必须具备来源、目录、命令和高风险确认边界。 |
| 完全访问被误解为无限授权 | 明确限定为当前项目 / 案件范围；SAP 写入、批量删除、外部发布和密钥导出仍需确认。 |
| 把 Codex 依赖换成另一种隐性依赖 | 核心 Agent Runtime、事件账本和可移植摘要由本项目维护；provider/Codex 只作适配器或参考。 |
| 长上下文导致成本、延迟和结论漂移 | 建立 token 预算、按需加载工具/Skill、可追溯压缩和 Top-K 检索。 |
| 记忆污染或跨客户串数据 | 记忆按 Project/Case 隔离、带来源、可编辑删除；知识仍需人工确认。 |
| Skill/MCP 提权 | 所有能力都经过 Tool Registry、Policy Engine、schema、路径和审批检查。 |
